...
def get_worksheet_safe(client, tab_name):
    # Yeh function check karay ga k user ne 'Purchase' likha hay ya 'Purchases'
    try:
        return client.worksheet(tab_name)
    except:
        # Agar 'Purchase' nahi mila tu 'Purchases' try karo
        try:
            return client.worksheet(tab_name + "s")
        except:
            st.error(f"Sheet '{tab_name}' nahi mili. Google Sheet mein tab ka naam check karein.")
            st.stop()

def get_users():
    try: 
        client = get_connection()
        ws = get_worksheet_safe(client, "User") # Users sheet try karein
...
def load_data(tab):
    try:
        ...
        # Use get_all_values instead of records (Fixes 200 Error)
    ...
    with st.form("buy_form", clear_on_submit=True):
        ...
        if st.form_submit_button("📥 Save Purchase"):
            ...
            if save_data("Purchase", [pk_time, party, final_weight_kg, rate, total_amt, details]):
                ...
                time.sleep(1) # YEH LINE AB SAHI CHALAY GI
                st.rerun()
