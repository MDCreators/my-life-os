...
# CSS: Hide Streamlit Branding & Style Cards
st.markdown("""
    <style>
...
if "Khareedari" in active_tab:
    ...
    with st.form("buy_form", clear_on_submit=True):
        ...
        # Weight Logic with Unit
        w_col, u_col = c2.columns([2,1])
        raw_weight = w_col.number_input("Wazan (Weight)", min_value=0.0, format="%.3f")
        unit = u_col.selectbox("Unit", ["Kg", "Grams"])
        
        rate = c3.number_input("Rate (Per Kg)", min_value=0)
        details = st.text_input("Tafseel (Details)")
        ...
        if st.form_submit_button("📥 Save Purchase"):
            ...
            save_data("Purchase", [pk_time, party, final_weight_kg, rate, total_amt, details])
            st.success("Saved!")
            st.rerun()
...
# === B. FAROKHT (SALE) ===
    ...
    st.header("🔴 Nayi Farokht (New Sale)")
    with st.form("sell_form", clear_on_submit=True):
        ...
        if st.form_submit_button("📤 Save Sale"):
            pk_time = datetime.now(pytz.timezone('Asia/Karachi')).strftime("%Y-%m-%d")
            save_data("Sale", [pk_time, cust, bill, final_weight_kg, rate, total_amt])
            st.success("Sold!")
            st.rerun()
...
# === C. CLOSING ===
    ...
    st.header("📒 Munafa / Nuqsan (Profit/Loss)")
    ...
    st.success("System auto-calculates profit based on Sale - Purchase.")

# === D. MANAGE USERS (ADMIN ONLY) ===
